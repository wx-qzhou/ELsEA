...
class ParallelEAFramework:
    def __init__(self, server: Server, clients: typing.List[Client], eval_way="sinkhorn", max_iteration=1):
        self.server = server
    ...
    def run(self):
        # self._prepare()
        # print("index {0}".format(1))

        # # partition G1
        # print("begin partitioning")
        # t11 = time.time()
        # # The following operator is employed to obtain the files, such as 'kg_partition.json' and 'tmp_part.json'.
        # # Here, 'kg_partition.json' denotes \hide(E)^{s,u}_i, and 'tmp_part.json' is P_i.
        ...
        for ite in range(0, self.max_iteration):
            ...
            for idx, client in enumerate(self.clients):
                # if idx <= 87:
                #     continue
                print("The id of client is {}.".format(idx))
                # train model
                t31 = time.time()
                ignore = client.generate_dataset_from_partition()
                if ignore: # This part does not have test data. Hence, it must be ignore.
                    self.ignore_ids.append(idx)
                    continue
                client.train_model()
                t32 = time.time()
                ...
                # evaluate model
                client.evaluate_model(self.eval_way)
            ...
            # communicate
            t51 = time.time()
            self.server.communicate(self.ignore_ids)  # collect msgs from clients and deliver msgs to clients
            t52 = time.time()
    ...
    def evaluate(self):
        print("index {0}".format(38))
        test_nums = []
        # self.ignore_ids += [36, 44, 60, 64, 76]
        for idx in range(len(self.clients)):
            if idx in self.ignore_ids:
                continue
        ...
        if self.eval_way == "sinkhorn":
            metrics_name = "metrics_sinkhorn"
        elif self.eval_way == "csls":
            metrics_name = "metrics_csls"
        elif self.eval_way == "cosine":
            metrics_name = "metrics_cos"

        for idx in range(len(self.clients)):
            if idx in self.ignore_ids: # no test data which is needed to ignore.
                continue
            ...
            with open(os.path.join(part_out_dir, "part_metrics.json")) as file:
                ...
                mrr_list.append(obj[metrics_name]["mrr"] * recall)
                recall1_list.append(obj[metrics_name]["hit@1"] * recall)
                recall5_list.append(obj[metrics_name]["hit@5"] * recall)
                recall10_list.append(obj[metrics_name]["hit@10"] * recall)
                recall50_list.append(obj[metrics_name]["hit@50"] * recall)
        test_nums = np.array(test_nums)
        test_percents = test_nums / np.sum(test_nums) # (task_num,)
        recall = np.matmul(test_percents, recall_list) # The shape is () and it is a number. 
