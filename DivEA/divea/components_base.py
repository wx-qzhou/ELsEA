...
from divea.dataload import write_tab_lines
...
class NeuralEAModule:
    ...
    def get_all_pred_alignment(self, eval_way="sinkhorn"):
        with open(os.path.join(self.conf.output_dir, "pred_alignment.json")) as file:
            ...
            all_pred_alignment = obj["all_pred_alignment"]
...
class Client:
    ...
    def evaluate_model(self, eval_way="sinkhorn"):
        print("index {0}".format(27))
        metrics_obj = self.ea_module.evaluate(eval_way)
        ...
        metrics_obj = {"recall": recall, "metrics_sinkhorn": metrics_obj["metrics_sinkhorn"], \
                       "metrics_csls": metrics_obj["metrics_csls"], "metrics_cos": metrics_obj["metrics_cos"]}
...
class ContextBuilder(object):
    def __init__(self, data: UniformData, data_dir, kg_ids, part_n, out_dir=None, g1_ctx_only_once=True):
        ...
        self.g2_ctx_neighbor_have_done = False
    ...
    def get_neighbours2(self, triples, part_entities, max_hop_k):
        ...
        for step in range(0, max_hop_k):
            tmp_triple_df = ent_df.merge(conn_df, how="inner", left_on="ent", right_on="h") # The head is (ent, h, t), and the shape of it is (N, 3).
            tmp_triple_df2 = ent_df.merge(conn_df, how="inner", left_on="ent", right_on="t") # The head is (ent, h, t), and the shape of it is (N, 3).
    ...
    def get_neighbours4(self, triples, part_entities, anchors, max_hop_k):
        ...
        for step in range(0, max_hop_k):
            if step>0 and step == max_hop_k-1:
                ...
                final_added_set = set(tmp_triple_df.values.reshape((-1)).tolist() + \
                    tmp_triple_df2.values.reshape((-1)).tolist()).difference(added_entity_set)
...
class G2Partitioner():
    ...
    #         with open(os.path.join(part_data_dir, "kg_partition.json")) as file:
    #             obj = json.loads(file.read())
    #         if "kg2_partition" not in obj:
    #             obj["kg2_partition"] = {}
    #         ctx_entities = []
    #         if "ctx_entities" in obj["kg2_partition"]:
    #             ctx_entities = obj["kg2_partition"]["ctx_entities"]
    #         obj["kg2_partition"] = {
    #             "entities": part_entities,
    ...
    def _process_partition(self, idx):
        try:
            part_idx = idx + 1
            print("partition g2: {0}/{1}".format(part_idx, self.part_n))
            part_entities = self._select_g2_candidates(part_idx)
            # part_triples = subgraph(part_entities, self.data.kg2_triples)
            # part_triples = self.data.kg2_sub_triples(part_entities)
            part_data_dir = os.path.join(self.data_dir, f"partition_{part_idx}")
    ...
    def partition_g2_entities(self):
        ...
        print("Partition g2 entities process num: ", pool_size)

        indices = range(1, self.part_n)  # 创建一个序列，从0到part_n-1
        # batch_size = 10
        # print("Partition g2 entities batch_size: ", batch_size)

        print("partition g2: {0}/{1}".format(0, self.part_n))
        self._process_partition(0)

        # with Pool(pool_size) as pool:
        #     if batch_size < self.part_n:
        #         num_batch = (self.part_n + batch_size - 1) // batch_size
        #         for i in range(num_batch):
        #             batch_indices = indices[i: (i + 1) * batch_size]
        #             pool.map(self._process_partition, batch_indices)
        #     else:
        #         pool.map(self._process_partition, indices)
        
        with Pool(pool_size) as pool:
            list(tqdm(pool.imap_unordered(self._process_partition, indices), initial=1, total=self.part_n, desc="Partitioning g2"))
